...
$rows = $conn->query("SELECT * FROM notifications WHERE user_id IS NULL ORDER BY created_at DESC LIMIT 20");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <?php while ($row = $rows->fetch_assoc()): ?>
      <article class="rounded-[2rem] border border-slate-800 bg-slate-900 p-6">
        <div class="flex items-center justify-between gap-4">
          ...
          <span class="text-sm text-slate-500"><?php echo h(date("d M Y, h:i A", strtotime((string) $row['created_at']))); ?></span>
        </div>
